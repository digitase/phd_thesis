...
or a stable \SI{100}{\percent} success rate in \SI{60}{\percent} of individuals and \SI{0}{\percent} in the other \SI{40}{\percent} (response is highly personal)---most likely the truth is somewhere in between.
...
In \cref{ch:hird_reQTL}, \software{xCell}---while not technically a deconvolution method---was used to estimate cell type enrichment scores from array and \gls{RNAseq} data.
...
For \textit{cis}-\gls{eQTL} mapping in \cref{ch:hird_reQTL,ch:multiPANTS}, where the main concern was maximising the number of \gls{eQTL} detected,
