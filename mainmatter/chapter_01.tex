...
Immunology began as study of host defense against infection, distinguishing self from non-self \autocite{murphy2016JanewayImmunobiology}.
The immune system is also involved in pathogenesis of allergic, autoimmune, and other immune-mediated diseases.
...
but delivers a powerful pathogen-specific response capable of eliminating pathogens that have evaded the innate response.
The adaptive response can also create immunological memory lasting years to decades,
where re-exposure to the same pathogen induces a faster and more powerful adaptive response%
\footnote{
...
Both arms involve complex interaction of many cell types through surface receptors and signalling molecules.
...
but certain types are more prominent in tissues or the lymphatic system.
...
but it is \gls{DC} that are the most specialised \glspl{APC}.
...
Antigen-presentation is the primary link between the innate and adaptive responses.
...
Naive lymphocytes express antigen receptors that recognise specific antigens.
...
CD4 (helper) T cells recognise antigen fragments presented in a complex with \gls{MHC} class II on the surface of \glspl{APC}.
% Binding requires TCR (CD3), CD4, MHC class II and costimulatory factors.
CD4 T cells then differentiate into several subsets that activate and regulate other cell types, including macrophages, CD8 T cells and B cells.
Activated CD8 (cytotoxic) T cells recognise antigens presented by \gls{MHC} class I on infected cells and directly kill the cell.
Activated B cells differentiate into plasma cells that secrete large quantities of antibodies that which identify and neutralise pathogens by specific binding to an epitope on the target antigen.
Antibody-mediated responses are also called humoral immunity,
whereas T cells and innate immune responses comprise cell-mediated immunity.
A small subset of activated B and T cells will become memory cells, responsible for immunological memory.
...
\enquote{systems immunology} proposes a holistic rather than reductionist approach \autocite{davis2017SystemsImmunologyJust,villani2018SystemsImmunologyLearning,pulendran2020ScienceMedicineHuman}.
...
A range of high-throughput technologies can be applied to measure response at many layers of the immune system (\autoref{fig:intro_sysImmunology}).
Longitudinal designs are common, profiling the transition from baseline into innate and adaptive response.
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth,page=1]{mainmatter/figures/chapter_01/1-s2.0-S2452310018301197-gr1/1-s2.0-S2452310018301197-gr1_lrg.jpg}
    \caption{Systems imm}
    \label{fig:intro_sysImmunology}
% TODO: "Reprinted from Publication title, Vol /edition number, Author(s), Title of article / title of chapter, Pages No., Copyright (Year), with permission from Elsevier [OR APPLICABLE SOCIETY COPYRIGHT OWNER]." Also Lancet special credit - "Reprinted from The Lancet, Vol. number, Author(s), Title of article, Pages No., Copyright (Year), with permission from Elsevier."
\end{figure}

There are three major aims of systems immunology.
...
Causal inference is the third and most difficult aim.
...
What interventions can be made to promote effective response to pathogens and vaccines,
and impede pathways that lead to immune dysregulation?
Obtaining a causal understanding is essential for clinical translation,
...
% \todo{"Immune response to vaccination" will be merged with ch2 intro.}
% \todo{"Immune response to anti-TNFs" will be merged with ch4 intro.}
%
...
\autoref{ch:hird_DGE} is a descriptive \gls{DGE} study of transcriptomic and antibody responses to pandemic influenza vaccine in the \gls{HIRD} cohort of healthy adults.
\autoref{ch:hird_DGE} integrates \gls{HIRD} genotype data to map the regulation of expression responses with an \textit{in vivo} \gls{reQTL} design.
\autoref{ch:hird_DGE} mirrors the design of the previous two chapters, 
exploring clinical response to biologic anti-\gls{TNF} therapy for \gls{CD} patients \gls{PANTS} cohort.
Finally, \autoref{ch:hird_DGE} presents an overview of shared themes and limitations,
