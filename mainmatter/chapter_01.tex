...
\textit{Trans}-\gls{eQTL} often manifest from \textit{cis}-\glspl{eQTL} on master regulatory molecules like \glspl{TF} and RNA-binding proteins that regulate many genes in \textit{trans} \autocite{fairfax2012GeneticsGeneExpression,albert2015RoleRegulatoryVariation}.
Gathering large enough samples to detect \textit{trans}-\glspl{eQTL} remains a priority,
as most expression heritability is driven by \textit{trans}- rather than \textit{cis}- effects \autocite{liu2019TransEffectsGene}.
% \2 Currently discovered bulk baseline cis-eQTL also don't explain a lot of complex trait heritability.
...
A non-exhaustive list of environmental contexts that interact with \gls{eQTL} effects includes:
    ...
    purified cell type \autocite{dimas2009CommonRegulatoryVariation,dejager2015ImmVarProjectInsights,peters2016InsightGenotypePhenotypeAssociations,chen2016GeneticDriversEpigenetic},
...
These contexts are interdependent.
...
\1 A variety of molecular mechanisms might facilitate genotype-environment interactions at \glspl{eQTL}.
\2 Most basic is not expressed
    \2 <list of mechanisms related to regulation of gene expression by TFs, enhancers, splice sites etc.>
        \3 \textcite{fu2012UnravelingRegulatoryMechanisms}: proposes classes of QTL and also TF-based mechanisms for cis-eQTL (here, define mag, damp, flip) (\cref{fig:intro_reQTLmechs})
for opposig fx: fairfax2012GeneticsGeneExpression
        \3 \autocite{ackermann2013ImpactNaturalGenetic}: defines static, conditional, additionally dynamic eQTLs
        ...
        % \3 also, priming at ca level?

% TODO: use fu2012UnravelingRegulatoryMechanisms figure
\begin{figure}
    \centering
...
\1 A seminal early study was conducted by \autocite{barreiro2012DecipheringGeneticArchitecture}, where eQTLs were mapped separately in monocyte-derived dendritic cells before and after 18h infection with \textit{Mycobacterium tuberculosis}.
    ...
    % schmiedel2018ImpactGeneticPolymorphisms
    % 	Found reQTLs? So what? Gather take home messages and inspiration for post reQTL analyses
        ...
        % o	context-dependent eQTL are identified because of both treatment-induced regulatory effects and treatment-inducing gene expression to detectable levels (Fairfax et al. 2014)
        % •	trans effects
...
As \glspl{eQTL} can propose both target gene and mechanism of action,
where the effect of variant on complex trait is mediated through expression,
they have proven to be a valuable data type for gene prioritisation.
\todo{I will mention that they also prioritise state e.g. cell type/tissues in the next subsections}
...
it should be supported by complementary lines of evidence \autocite{hemani2018EvaluatingPotentialRole} to help untangle the multiplex of possible causal pathways from variant to trait.
...
\1 Given the effect of an eQTL can be starkly different between environments, it is critical to use trait-relevant \gls{eQTL} datasets for target gene prioritisation at GWAS loci.
    \2 eQTL enrichment in GWAS hits,
        \3 esp in context specific \autocite{kim-hellmuth2020CellTypeSpecific}
    \2 It has already been shown that use of relevant cell-type specific eQTLs increases coloc rates with GWAS hits \autocite{kim-hellmuth2020CellTypeSpecific} \url{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4498151/} \url{https://www.biorxiv.org/content/10.1101/2020.01.15.907436v1}
