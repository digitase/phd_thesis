...
To estimate the appropriate shape and scale parameters for the gamma empirically, a frequentist random-effects model using the \gls{REML} estimator for $\tau$ (recommended for continuous effects \autocite{veroniki2016MethodsEstimateBetweenstudy}) was fit for each gene using \software{metafor::rma.uni} \autocite{viechtbauer2010ConductingMetaAnalysesMetafor}.
Genes with small estimates of $\tau < 0.01$ were excluded, and a gamma distribution was fit to the remaining estimates using \software{fitdistrplus} \autocite{delignette-muller2015FitdistrplusPackageFitting}.
...
For the frequentist random-effects meta-analysis, nominal gene-wise \pvalues{} are converted to \gls{FDR} estimates using \software{p.adjust(method='BH')} in R.
...
Rank-based gene set enrichment analysis using \software{tmod} \autocite{weiner3rd2016TmodPackageGeneral} revealed that genes with the highest \gls{FC} increases at day 1 were enriched in modules associated with activated \glspl{DC}, monocytes, \glspl{TLR}, and inflammatory signalling (\cref{fig:hird_tmodDotPlot_timepoint}).
...
I tested for association with a baseline-adjusted, continuous measure of antibody response, the \gls{TRI} \autocite{bucasas2011EarlyPatternsGene}.
The \gls{TRI} was comparable to the binary definition (\cref{fig:hird_tri}g, \cref{fig:hird_tri}h), but improves statistical efficiency to detect associations.

A frequentist random-effects meta-analysis was initially run.
...
Of the day 7-specific associations reported by \autocite{sobolev2016AdjuvantedInfluenzaH1N1Vaccination},
58/62 fell into the \num{13593} genes considered in my meta-analysis (circled in \cref{fig:hird_DGE_effectSizeComparisons_rma}h), and 15/58 replicated, all with the same positive direction of effect (high expression with high \gls{TRI}).
However, these significant results are driven more by higher effect sizes in the array compared to the \gls{RNAseq} samples, with the majority of the 15 genes lying above the diagonal.
This is in contrast to the associations identified with timepoint, where significant genes have mostly consistent effects between platforms (\cref{fig:hird_DGE_effectSizeComparisons_rma}b,c,d).

In the Bayesian meta-analysis, 
which more robustly models between-platform heterogeneity with an empirical prior, 
no single gene was detected as significantly associated with \gls{TRI} at $\text{\gls{lfsr}} < 0.05$ at any single timepoint, or when pooling samples across all timepoints (\cref{fig:hird_DGE_effectSizeComparisons_bayesmeta}e,f,g,h).
The frequentist meta-analysis is likely to use poor estimates of the between-platform heterogeneity, as there are only being two input to estimate it from.
Indeed, all 15 genes associated with \gls{TRI} in the frequentist meta-analysis
had unrealistic between-platform heterogeneity estimates of exactly zero
...
\begin{figure}
    ...
    \caption{
        ...
        All significant results occur when the estimate is zero.
        Estimates from the Bayesian method are in a narrow range and constrained away from zero by the empirical prior.
    }
    \label{fig:hird_DGE_sobolev2016hits_tauComparison}
\end{figure}

Using effect sizes from the Bayesian meta-analysis,
significant enrichments were detected at the gene set level;
the strongest effects were seen at day 7, where expression of modules related to 
the cell cycle (e.g. LI.M4.1, LI.M4.2, LI.M4.0), 
cell division in CD4\textsuperscript{+} T cells (e.g. LI.M46, LI.M4.5), 
and plasma cells (e.g. LI.S3, LI.M156.1) were associated with high \gls{TRI} (\cref{fig:hird_tmodDotPlot_TRI}).
Associations with \gls{TRI} were also detected at baseline, notably for
modules related to proinflammatory processes.
These include 
\enquote{chemokines and inflammatory molecules in myeloid cells} (LI.M86.0, $\text{\gls{FDR}} = \num{2.252190e-11}$),
\enquote{proinflammatory cytokines and chemokines} (LI.M29, $\text{\gls{FDR}} = \num{1.216638e-10}$),
and \enquote{myeloid, dendritic cell activation via NFkB (I)} (LI.M43.0, $\text{\gls{FDR}} = \num{4.675413e-07}$),
all having positive association with antibody response.
...
% discussion bait
% most change at day 1, consistent with seasonal and hird
% match the dcrease in cell frequ of T cells at day 1
% and heavy incrase in mono esp cd16+

% lessons: this is same cohort, diff tech
% between cohorts,
% single gene may nto be robust, esp is the effect sizes ar small vs between platform noise
% like with TRI, not timepoint

% general pattern consistent with sys vacc
% particularly with
% nakaya2016SystemsBiologyImmunity ATIV consistent d1
%
% with TRI,
% nakaya2015SystemsAnalysisImmunity baseline consistent

There is extensive transcriptomic response to Pandemrix vaccination in the \gls{HIRD} cohort.
Upregulation of genes and modules related to the interferon signalling, monocytes, inflammatory response, and other aspects of innate immunity were detected at day 1.
...
% TODO: not adjusted for cell comp
% We do not know if upreg or recruit
% can adjust, using HIRD FACS, or by deconv in ch 3
% or PBMC stim in vitro with vaccine \autocite{querec2009SystemsBiologyApproach}
