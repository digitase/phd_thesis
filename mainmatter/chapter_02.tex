...
% The four genera of Influenza virus that infect vertebrates, which are identified by antigenic differences in their nucleoprotein and matrix protein.
Human infections are primarily caused by the influenza A and influenza B virus types.
Each year, seasonal epidemics result in \textapprox{1} billion infections---mostly in children, driving transmission---and \numrange{300000}{500000} deaths worldwide.
Peak seasonality is defined by low humidity, low temperature, and other climate factors.
Risk factors for severe illness and death include extremes of age (infants \SI{<1}{\year}, elderly \SI{>65}{\year}), pregnancy, obesity, chronic illness, and host genetics (e.g. mutations in \gene{IFITM3} and \gene{IRF7}) \autocite{krammer2018Influenza}.
...
Influenza viruses are enveloped viruses with a negative-sense single-stranded RNA genome divided into segments (eight segments for influenza A and B), each encoding one or more viral proteins.
% Stalk is more conserved.
...
\gls{HA}, with its characteristic head-stalk structure, facilitates viral entry by binding sialic acid-containing receptors on host cells.
\gls{NA} facilitates viral release, cleaving sialic acid to prevent binding of newly-synthesised viruses back to the dying host cell in the final stages of the viral life cycle.
The gradual accumulation of mutations in these surface protein genes, especially in the \gls{HA} head domain, is known as antigenic drift,
and can lead to evasion of antibody-mediated immunity acquired during previous exposures.
At least 18 \gls{HA} subtypes and 11 \gls{NA} subtypes---all antigenically-distinct---have been defined for influenza A, the virus type with the greatest prevalence, host range, and genetic diversity \autocite{krammer2018Influenza}.
...
Each was caused by influenza A, involving reassortment of human and animal strains or zoonotic transmission \autocite{short2018BackFutureLessons}.
...
%
The most recent pandemic was due to an influenza A with \gls{HA} subtype 1 and \gls{NA} subtype 1 gene segments from avian and swine lineages \autocite{garten2009AntigenicGeneticCharacteristics}: A(H1N1)pdm09%
\footnote{
...
\subsection{Influenza vaccines}

Vaccination is the primary method for prevention and control of influenza.
Antigenic drift and decline of vaccine-induced immunity over time means repeated vaccination is required.
% NOTE: strain-specific antibody responses against natural infection are typically long lasting though
Seasonal vaccines are multivalent, formulated against three (trivalent) or four (quadrivalent) influenza strains anticipated circulate in upcoming seasons.
\gls{WHO} surveillance programs make recommendations on the most representative strains for the Northern and Southern hemispheres each year.
...
% Egg based vaccines take more time to grow, during which mismatches between vaccine strains and circulating virus strains can occur.
\Gls{LAIV} contain low-virulence, cold-adapted viruses that replicate well only in the cool upper respiratory tract.
Recombinant \gls{HA} vaccines contain purified recombinant \gls{HA} expressed in insect cell lines rather than relying on viral propagation in embryonated chicken eggs, hence are faster to manufacture.
...
Efficacy of licensed vaccines is \SIrange{50}{90}{\percent} on average in healthy adults,
depending greatly on vaccine class, the match between vaccine and circulating strains, and host factors \autocite{dhakal2019HostFactorsImpact}.

% Different vaccines for different people
% different classes, adjuvants etc.

\subsection{Immune response to influenza vaccines}

Both \glspl{IIV} and recombinant \gls{HA} vaccines primarily induce anti-\gls{HA} serum IgG antibody responses that protect against reinfection.
\Glspl{LAIV} additionally induce mucosal IgA and T cell responses, more closely approximating natural infection.
% IgA may lower shedding and transmission? T cell responses help with recovery?
...
% Nevertheless, haemagglutination
% inhibition titres have been established as a correlate
% of protection and are widely accepted as a readout for
% vaccine efficacy studies by regulatory agencies11,78,79. It is
% assumed that a 1:40 titre of antibodies with haemagglutination
% inhibition activity will reduce the risk of getting
% an infection with seasonal H1N1 virus, H3N2 virus
% or influenza B viruses by 50%.
...
% TODO: original antigenic sin
% Typically,
% exposure to a new virus variant (or subtype) might
% also induce antibodies to viruses encountered earlier
% in life, depending on the relatedness of the two antigens.
% An antibody response against a novel virus variant in
% adults might, therefore, be heavily influenced by the
% individual’s exposure history117,127,128, which is an issue
% for vaccine development.
%
% krammer2019HumanAntibodyResponse
% proteins. Therefore, first exposure to the
...
% TODO: on pandemrix efficacy from it's trials
...
% NOTE: this is the standard WHO name https://www.who.int/influenza/gisrs_laboratory/terminology_ah1n1pdm09/en/
...
\footnote{
    The \gls{WHO} nomenclature for isolates specifies influenza type (A, B, C, D), host of origin (human if omitted), geographical origin, strain number, year of isolation, and isolate subtype (combination of \gls{HA} and \gls{NA} subtypes) \url{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2395936/}.
}
...
% TODO: why PBMC fraction? list important cell types here
...
% TODO: 4-fold less likely to acheive by high baseline, for biological reasons, also RTM, also the forced negative correlation by cov formula
...
% Checking that I understand the sources of technical variability, is it that all the samples in each pool will go through library prep together (e.g. same plate, reagent batch, roughly same time), and then be barcoded and sequenced on the same (lane of the) Novaseq flow cell?
...
% TODO: define composition bias that TMM corrects, see evans2018SelectingBetweensampleRNASeq, pull from ch5
