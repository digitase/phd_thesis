...
Coefficients and their standard errors were extracted from the linear models.
...
\todo{make all the notation in this section consistent with 2.1. The normal-normal hierarchical model, \autocite{rover2017BayesianRandomeffectsMetaanalysis}}
...
Models were fit on per-gene, per-platform, then at 13593 genes with expression measured by both platforms, effect sizes were combined using Bayesian random-effects meta-analysis.

At a \gls{lfsr}$ < 0.05$ cutoff, 5807/13593 genes were significantly differentially expressed post-vaccination compared to baseline, falling into three groups: 2865 genes upregulated at day 1, 2748 genes downregulated at day 1, and 194 genes upregulated at day 7 (\autoref{fig:hird_dge_heatmap}).
% At a cutoff of absolute $\text{fold-change} > 0.5$ and \gls{lfsr}$ < 0.05$, 703 genes were DE. Expression of these genes cluster into three main patterns: 580 genes upregulated at day 1, 64 genes downregulated at day 1, and 59 genes upregulated at day 7 (\autoref{fig:hird_dge_heatmap}).
 
\begin{figure}
    \includegraphics[width=1.0\textwidth]{mainmatter/figures/chapter_02/plot_dge_eqtl.heatmap_dge.pdf}
    \caption{Normalised gene expression for differentially expressed genes ($\text{lfsr} < 0.05$, $\left|\text{FC}\right| > 1.5$) across \gls{HIRD} samples from baseline, day 1, and day 7, clustered by gene.}
    \label{fig:hird_dge_heatmap}
\end{figure}

\subsection{Innate immune response at day 1 post-vaccination}

Consistent with global expression at day 1 being markedly different from expression at other timepoints (\autoref{fig:hird_expression_pcs}), the highest numbers of differentially expressed are observed for the day 1 vs. baseline comparison.
The gene with the highest \gls{FC} increase at day 1 was \textit{ANKRD22} ($\log_2\text{\gls{FC}} = 4.489150$), an interferon-induced gene in monocytes and \glspl{DC} involved in antiviral innate immune pathways\autocite{bin2016AnkyrinRepeatDomain}.
...
\todo{add automatic rounding to x decimal places}
Gene set enrichment analysis using \texttt{tmod} revealed that genes with the high \gls{FC} increases at day 1 were enriched in modules associated with activated \glspl{DC}, monocytes, toll-like receptor and inflammatory signalling (\autoref{fig:hird_tmodDotPlot_day}), confirming that day 1 responses are dominated by signatures of innate immunity.
\todo{can also add MSigDB hallmark sets, which include interferon sets; and of course gene ontology sets}
\todo{consider commenting on transience of response i.e. genes are then downreg at day 7. Would need to add another annotation col to heatmap, and tmod dotplot}
\todo{any point in a table of e.g. top 20 DE genes, or is the gene set analysis already enough?}

\begin{figure}
    \includegraphics[width=1.0\textwidth]{mainmatter/figures/chapter_02/compare_dge_eqtl.tmodDotPlot.DGE.day.pdf}
    ...
    \label{fig:hird_tmodDotPlot_day}
\end{figure}
...
The enriched gene sets at day 7 were related to mitosis and cell proliferation, particularly in CD4\textsuperscript{+} T cells (\autoref{fig:hird_tmodDotPlot_day}).
Both the CD4\textsuperscript{+} T cell and plasma cell response are indications of an adaptive immune response at day 7.\todo{add reference to role of CD4s in e.g. class switching of B cells}

\subsection{Expression signatures associated with antibody response}

We do not replicate 
\autoref{fig:hird_DGE_platform_effectSizeComparisons}

\begin{figure}
    \includegraphics[width=1.0\textwidth]{mainmatter/figures/chapter_02/plot_dge_eqtl.effectSizeComparison.pdf}
    \caption{DGE effect sizes estimated in array vs \gls{RNAseq}.}
    \label{fig:hird_DGE_platform_effectSizeComparisons}
\end{figure}

We do find signal at gene set level
\autoref{fig:hird_tmodDotPlot_TRI}

\begin{figure}
    \includegraphics[width=1.0\textwidth]{mainmatter/figures/chapter_02/compare_dge_eqtl.tmodDotPlot.DGE.R.vs.NR.pdf}
...
\end{figure}
\todo{x labels here should be TRI, not R.vs.NR}

% \subsection{Comparison to Sobolev}
...
\todo{analysis not complete, possibly not required for this chapter}
