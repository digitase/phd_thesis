...
% TODO:
% collab note from first year report acknowledgements
% data from HIRD
% anna lorenc
% sean
% adrian
...
%
% And of course, sobolev2016AdjuvantedInfluenzaH1N1Vaccination
...
Genes associated with both myeloid and lymphoid effector functions had increased expression at day 1 versus baseline, most prominently for genes associated with interferon responses.
...
These early myeloid responses were consistent with studies of unadjuvanted seasonal influenza vaccines, 
but a type II interferon gamma-associated lymphoid response was unique to this adjuvanted vaccine was observed.
\todo{add age effect on early response}

Vaccine response was defined as a \num{\ge 4}-fold increase in either \gls{HAI} or \gls{MN} titres after vaccination.
...
\begin{itemize}
    ...
    \item and marker deviation from \gls{HWE}, an indication of genotyping or genotype calling errors \autocite{mccarthy2008GenomewideAssociationStudies,anderson2010DataQualityControl} (\texttt{-{}-hwe} option, \pvalue{} \SI[scientific-notation=true]{< 0.00001})%
        \footnote{
...
Genotypes were first \gls{LD}-pruned (PLINK \texttt{-{}-indep-pairwise 50 5 0.2} i.e. in a sliding window of \SI{50}{\kilo\bp}, step size of 5 variants, remove variants at each step until no pair of variants has \gls{LD} \num{>0.2}), 
...
Relative transcript abundances were summarised to Ensembl gene-level (release 90) count estimates using tximport (\texttt{scaledTPM} method, which scales Salmon \glspl{TPM} up to the library size \autocite{soneson2016DifferentialAnalysesRNAseq,love2018SwimmingDownstreamStatistical}) to improve statistical robustness and interpretability \autocite{soneson2016DifferentialAnalysesRNAseq}.
...
%
\gls{RNAseq} produces compositional data due to sequencing a fixed number of reads per library; if one gene's expression goes up that a library, another's must go down.
It is important to account for composition bias, the dependence of expression estimates on the expression properties of other genes in the library \autocite{robinson2010ScalingNormalizationMethod}.
...
Precision weights for each (gene by sample) observation were computed with voom\autocite{law2014VoomPrecisionWeights} to account for the mean-variance relationship in \gls{RNAseq} data;
voom also transforms expression values to the $\log_2{\text{\gls{CPM}}}$ scale, accounting for composition bias using effective library sizes.
...
%
Single-channel Agilent 4x44K expression array (G4112F) data for 173 samples from \textcite{sobolev2016AdjuvantedInfluenzaH1N1Vaccination} were downloaded from ArrayExpress (\url{https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-2313/}).
...
\texttt{VSN::normalizeVSN} \autocite{huber2002VarianceStabilizationApplied} was used to simultaneously perform 
...
Probes were matched to genes using \texttt{hgug4112a.db}\footnote{\url{https://bioconductor.org/packages/release/data/annotation/html/hgug4112a.db.html}}.
Most genes were targeted by multiple array probes; \num{31208} probes were collapsed into \num{18216} Ensembl genes using by selecting the probe with the highest mean intensity for each gene (\texttt{WGCNA::collapseRows(method=MaxMean)}, a method recommended for probe to gene collapsing by \textcite{miller2011StrategiesAggregatingGene}).
...
Finally, \texttt{limma::arrayWeightsQuick} \autocite{ritchie2006EmpiricalArrayQuality} was used to compute per-sample quality weights used to downweight unreliable arrays (samples) in the \gls{DGE} analyses.
...
% NOTE: only relevant to fold changes
% For example, arrays suffer from ratio compression due to cross-hybridisation \autocite{draghici2006ReliabilityReproducibilityIssues,ma2017JointBayesianModel}.
...
Within-individual correlations for the random effect were estimated using \texttt{limma::duplicateCorrelation}.
...
Corresponding coefficients and standard errors for the contrasts were extracted from the \texttt{limma} models, 
...
so the assumption of $\tau=0$ is unrealistic, and a random-effects model is more appropriate.
...
For this study, I use the implementation in \texttt{bayesmeta} \autocite{rover2017BayesianRandomeffectsMetaanalysis}.
...
To estimate the appropriate shape and scale parameters for the gamma empirically, a frequentist random-effects model using the \gls{REML} estimator for $\tau$ (recommended for continuous effects \autocite{veroniki2016MethodsEstimateBetweenstudy}) was fit for each gene using \texttt{metafor::rma.uni} \autocite{viechtbauer2010ConductingMetaAnalysesMetafor}.
Genes with small estimates of $\tau < 0.01$ were excluded, and a gamma distribution was fit to the remaining estimates using \texttt{fitdistrplus} \autocite{delignette-muller2015FitdistrplusPackageFitting}.
...
Since \texttt{bayesmeta} does not implement a Cauchy prior, to avoid over-shrinkage, I flatten the normal prior considerably by scaling up the standard deviation by a factor of 10: $N(0, (10\sigma)^2)$.
...
\begin{figure}
    ...
    \caption{
        \textbf{Gamma prior for $\tau$ (blue) used for \texttt{bayesmeta} analyses of the day 1 vs. baseline effect, compared to the empirical distribution of per-gene frequentist \texttt{metafor::rma.uni} estimates for $\tau$.}
...
\begin{figure}
    ...
    \caption{
        \textbf{Normal prior for $\mu$ (blue) used for \texttt{bayesmeta} analyses of the day 1 vs. baseline \gls{DGE} effect, compared to the empirical distribution of per-gene frequentist \texttt{metafor::rma.uni} estimates for $\tau$.}
        Genes with small estimates of $\tau < 0.01$ were first excluded.
        The original non-scaled normal fit is shown (black), as well as a Cauchy fit (red).
        Distribution parameters are listed.
        Alternative estimate of the Normal standard deviation more robust to outliers using a quantile matching method from \texttt{DESeq2} \autocite{love2014ModeratedEstimationFold} is also shown.
        In this case, it was comparable to the \gls{ML} estimate from \texttt{fitdistrplus}.
    }
...
For per-platform \gls{DGE}, \gls{FDR} control was done with \texttt{limma::decideTests} using the \gls{BH} procedure.
For the frequentist random-effects meta-analysis, nominal gene-wise \pvalues{} are converted to \gls{FDR} estimates using \texttt{p.adjust(method='BH')} in R.
...
%
For the Bayesian random-effects meta-analysis, posterior effect sizes and standard errors are supplied to \texttt{ashr}, which estimates the \glspl{lfsr} under the assumption that effects are unimodal.
% TODO: what about symmetry?
\gls{lfsr} is analogous to \gls{FDR}, but quantifies the probability of calling the wrong sign for an effect rather than than the confidence of a non-zero effect\autocite{stephens2016FalseDiscoveryRates}.
...
Gene set enrichment analyses were conducted using \texttt{tmod::tmodCERNOtest} \autocite{weiner3rd2016TmodPackageGeneral}, 
...
The effect size is given by the \gls{AUC}.
...
Finally, \gls{AUC} is $U/(nN)$, which takes values from 0.5 to 1.
Since \texttt{tmod::tmodCERNOtest} only considers enrichment of small ranks, but genes can be down or upregulated, 
...
At the \num{13593} genes with expression measured by both platforms, models were fit within each platform, then effect sizes were combined using Bayesian random-effects meta-analysis.
...
Gene set enrichment analysis using \texttt{tmod} revealed that genes with the high \gls{FC} increases at day 1 were enriched in modules associated with activated \glspl{DC}, monocytes, toll-like receptor and inflammatory signalling (\cref{fig:hird_tmodDotPlot_timepoint}), confirming that day 1 responses are dominated by innate immunity.
...
\subsection{Expression modules associated with antibody response}
...
\subsection{Identifying expression modules for predicting antibody response [probably cut this section and just add to discussion]}
...
This indicate a shift to the adaptive immune response, involving CD4\textsuperscript{+} T cell-supported differention and proliferation of antibody-secreting plasmablasts and plasma cells\autocite{murphy2016JanewayImmunobiology}.
...
% concur with max change at day 1 \autocite{bucasas2011EarlyPatternsGene}

% Point is, it's consistent over time, but i do more
...
Expression associations with antibody response were, however, observed at the gene set level, for modules of coexpressed genes that are associated with \gls{TRI} as a whole.
...
It should be noted that comparisons with existing influenza systems vaccinology studies should caveated, as most existing studies are for non-adjuvanted influenza vaccines.
...
% plotkins ch31 inactivated
% serum antibody levels peak 2 to 4 weeks after vaccination in healthy, previously primed individuals, but may peak 4 weeks or later in unprimed individuals and the elderly.480, 482, 483, 484, 485
...
Hence, it is particularly important that the robustness of these observed baseline expression associations be validated in an independent cohort for a comparable AS03-adjuvanted influenza vaccine.
...
%
% tsang2014GlobalAnalysesHuman
% Neither
% day 0 gene expression nor pathway activity (defined in Figure
% S2C) alone was predictive of either endpoint, though a
% number of robustly correlated genes and pathways could be
% identified (Figure 6A). Notably, genes associated with pattern
% recognition and interferon signaling robustly correlated to MFC
% but less so to adjMFC.
...
% krammer2018Influenza for T cell response
