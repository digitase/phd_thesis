...
Most genes are covered by multiple array probes; 31208 probes were collapsed into 18216 Ensembl genes using by selecting the probe with the highest mean intensity for each gene (WGCNA::collapseRows, using the recommended MaxMean method)\autocite{miller2011StrategiesAggregatingGene}.
...
Why is there a platform effect
different tech -> diff statistical models

\gls{RNAseq} has a higher dynamic range, 
...
Regarding the microarray batch effect, a popular adjustment method is ComBat\autocite{johnson2007AdjustingBatchEffects}, which estimates centering and scaling parameters per-gene pooling information across all genes using empirical Bayes.
...
However, where batches are unbalanced in terms of sample size\autoref{zhang2018AlternativeEmpiricalBayes} or distribution of study groups that have an impact on expression\autoref{nygaard2015MethodsThatRemove}, ComBat can overcorrect batch differences or bias estimates of group differences respectively.
...
\subsubsection{\glsfmtlong{DGE} meta-analysis}

The meta-analysis is restricted to 13945 genes assayed by both the array and \gls{RNAseq} platforms.
...
combining platforms is fraught with difficulties.
...
\subsubsection{Bayesian meta-analysis model}
...
%grepl(results.tables$d7.vs.d0 %>% head(100) %>% select(GENENAME)%>% unlist, pattern='^IG') %>% sum
...
%The overrepresented Gene Ontology categories amongst upreglated genes included "mitotic cell cycle" (p = 8.458339e-21, 2.785339e-22; cell division 1.586245e-16, 2.843928e-19, chromosome segregation, 6.803772e-15, 1.777793e-19".
