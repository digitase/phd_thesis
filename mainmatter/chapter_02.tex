...
\begin{figure}
    ...
    \caption{
        \textbf{Raw foreground intensities for 173 \gls{HIRD} array samples.}
...
\begin{figure}
    ...
    \caption{
        \textbf{Array intensity estimates after normalisation and collapsing of probes to genes.}
        Colored by array processing batch.
    }
...
As \textcite{sobolev2016AdjuvantedInfluenzaH1N1Vaccination} found there was no global dissimilarity in array expression between day -7 and day 0,
for the \gls{DGE} analyses in this chapter, 
array day -7 and day 0 are treated as repeated baseline measurements.

For each gene and platform, I fit a model (model 1) with expression as the response variable; with timepoint (baseline, day 1, day 7), \gls{TRI}, batch/pool, sex, age, and the first 4 genotype \glspl{PC} as fixed-effect predictors; and individual as a random-effect predictor.
...
Given k studies, the fixed-effect model assumes a common population effect size shared across all studies $\mu$, with observed variation explained only by sampling error $\sigma_i$, where $i = 1, \dots, k$.
% TODO: rover notation, hyperparameters
The random-effects model assumes the k study-specific effect sizes are drawn from some distribution with variance $\tau^2$ (standard deviation $\tau$), representing an additional source of variation termed the between-studies heterogeneity, reducing to the fixed-effect model when $\tau=0$.
In the \gls{HIRD} data, there are $k=2$ studies: array and \gls{RNAseq}.
The between-platform differences described in \cref{subsubsec:hird_dge_platform_and_batch_effects} represent considerable between-studies heterogeneity,
...
For this study, I use the implementation in \texttt{bayesmeta} \autocite{rover2017BayesianRandomeffectsMetaanalysis}, which requires priors for both effect size and between-studies heterogeneity.
...
% TODO: fdr separate option see limma manual 13.3 Multiple Testing Across Contrasts
