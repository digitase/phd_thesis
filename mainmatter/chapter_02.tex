...
\subsection{Immune response to influenza vaccines}
...
\subsection{Systems studies, and the \glsfmtfull{HIRD} study}

\begin{outline}

% Systems vaccinology is the analysis of high-dimensional -omics data measured at multiple levels of the biological system, in order to characterise response to vaccination (Fig.~\ref{fig-sys-vacc-overview}).

%     The various types of seasonal influenza vaccine have also been well-studied.
% Zhu et al.\autocite{Zhu2010} found that expression of type 1 interferon-modulated genes was a biomarker of early response to live attenuated influenza vaccine.
% An expression signature including STAT1, CD74, and E2F2 correlated with serum antibody titres after vaccination with trivalent inactivated influenza vaccine\autocite{Bucasas2011}, and kinase CaMKIV expression is also a strong predictor \autocite{Nakaya2011}, as are genes related to B cell proliferation \autocite{Tan2014}.

% Summarise sobolev
    %See references "Importantly, and in contrast to flu vaccines used in US immune-monitoring studies8,13,15–17" from Sobolevs
% When studying seasonal influenza vaccines in adults, responses tend to include recall from past vaccination or infection\autocite{Bucasas2011,Nakaya2012a}.
% The Human Immune Response Dynamics (HIRD) study conducted by Sobolev et al.\autocite{Sobolev2016} was designed to monitor response to a strain of H1N1 `swine flu' influenza, against which the majority of the cohort would be unlikely to have immunological memory.
% The longitudinal study consisted of 178 patients vaccinated with a single dose of AS03-adjuvated influenza-H1N1 vaccine.
% Gene expression was profiled using a microarray in a subset of 46 individuals.
% Upregulation of transcripts associated with both myeloid and lymphoid effector functions was observed by day 1, most prominently for transcripts associated with type I and type II interferons.
% Plasmablast-related differential expression between 23 vaccine responders and 18 non-responders was detected at day 7 post-vaccination.
% However, due to high variability amongst vaccine non-responders, a predictive model that segregated the two groups could not be built, even considering other measures such as frequencies of immune cell subsets and serum cytokine levels, suggesting there was no single contributing factor that led to vaccine failure.
...
\begin{outline}
    ...
    \1 Rationale for our study:
        % \item Extend the analyses of Sobolev et al.\autocite{Sobolev2016} by conducting RNA-seq on members of the HIRD cohort lacking microarray expression data, then perform differential gene expression analyses between the sampled timepoints, and between vaccine responders and non-responders, in order to characterise the transcriptomic contribution to response.
    %     \item Apply summary statistic meta-analysis techniques to combine the original microarray data with our RNA-seq data, increasing power for the above analyses.
    % \item Attempt to build an integrative model that predicts the quality of an individual's antibody response based on genetic, transcriptomic, and other available measures.
...
Adjuvanted influenza vaccines are considerably more immunogenic, and post-vaccination expression patterns differ to those of non-adjuvanted vaccines (\url{https://www.frontiersin.org/articles/10.3389/fimmu.2017.01760/full}).
