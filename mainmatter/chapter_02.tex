...
\textit{
    ...
    Sean O'Farrell and Anna Lorenc, for providing the HIRD clinical, FACS, and antibody titre data, and for providing advice on the data formats;
...
% e.g. only trivalent injectable influenza vaccine available 10 years ago
...
\subsection{Existing HIRD data and additional data generation}
...
\begin{figure}
    ...
    \caption[
        \captionshort{Overview of \gls{HIRD} study data.}
    ]{
        \textbf{Overview of \gls{HIRD} study data.}
        The total cohort size was 178 individuals.
...
\begin{figure}
    ...
    \caption[
        \captionshort{Antibody titre data and responder definitions.}
    ]{
...
Although the proportion of responders between array (32/44) and \gls{RNAseq} (59/70) individuals is similar ($p = \num{0.1551}$, Fisher's exact test), the variance of \gls{TRI} in array individuals is higher ($p = \num[scientific-notation=true]{0.0002098}$, Levene's test), suggesting more extreme antibody response phenotypes are present (\cref{fig:hird_phenotypes_by_platform}).
% NOTE:
...
\begin{figure}
    ...
    \caption[
        \captionshort{Distribution of patient \glspl{TRI}, stratified by expression measurement platform.}
    ]{
        \textbf{Distribution of patient \glspl{TRI}, stratified by expression measurement platform.}
        Points are colored by binary response status.
    }
...
\begin{figure}
    ...
    \caption[
        \captionshort{Sample filters for marker missingness and marker heterozygosity rate.}
    ]{
...
\subsection{Computing genotype principal components as covariates for ancestry}
...
\begin{figure}
    ...
    \caption[
        \captionshort{\gls{HIRD} samples projected onto \gls{PC} axes defined by \gls{PCA} of HapMap 3 samples.}
    ]{
        \textbf{\gls{HIRD} samples projected onto \gls{PC} axes defined by \gls{PCA} of HapMap 3 samples.}
...
\begin{figure}
    ...
    \caption[
        \captionshort{FastQC per-base sequence quality (Phred scores) versus read position for \gls{RNAseq} samples.}
    ]{
...
%     \caption[
%     ]{
%         FastQC sequence duplication levels for \gls{HIRD} \gls{RNAseq} samples.
%     }
...
\begin{figure}
    ...
    \caption[
        \captionshort{FastQC per-read GC distributions for \gls{RNAseq} samples.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Distributions of removed short \gls{ncRNA} and globin counts as a proportion of total counts in \gls{RNAseq} samples.}
    ]{
        \textbf{Distributions of removed short \gls{ncRNA} and globin counts as a proportion of total counts in \gls{RNAseq} samples.}
    }
...
\begin{figure}
    ...
    \caption[
        \captionshort{Distribution of the proportion of samples in which genes were detected (non-zero expression).}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Distributions of gene expression for \gls{RNAseq} samples before and after filtering low expression and non-detected genes.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Distribution of raw foreground intensities for \gls{HIRD} array samples ($n=173$).}
    ]{
        \textbf{Distribution of raw foreground intensities for \gls{HIRD} array samples ($n=173$).}
...
\software{VSN::normalizeVSN} \autocite{huber2002VarianceStabilizationApplied} was used for simultaneous 
...
and variance-stabilisation of intensity values (generalised logarithm, similar to $\log_2$ with better performance for small values), 
...
\begin{figure}
    ...
    \caption[
        \captionshort{Distribution of per-sample expression estimates after normalisation and collapsing of probes to genes.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{First four standardised \glspl{PC} in the expression data, colored by array batch/\gls{RNAseq} pool (a, c), timepoint (b, d), and binary response status (e, f).}
    ]{
        \textbf{First four standardised \glspl{PC} in the expression data, colored by array batch/\gls{RNAseq} pool (a, c), timepoint (b, d), and binary response status (e, f).}
...
\subsubsection{Per-platform DGE model}
...
\subsubsection{Choice of DGE meta-analysis method}
...
These are distributed around $\mu$ with variance $\tau^2$ (standard deviation $\tau$), representing an additional source of variation: the between-study heterogeneity.
Then we have 
...
\footnote{
    ...
    The required assumption is exchangeability of study \emph{effects}, which informally states
    % IID (independent, identically distributed) -> exchangeability -> identically distributed
...
\subsubsection{Prior for between-study heterogeneity}

The choice of prior for between-study heterogeneity $\tau$ is influential when $k$ is small \autocite{seide2019LikelihoodbasedRandomeffectsMetaanalysis}.
...
% TODO why is this? is it having well powered studies? Gelman is vague on this point
...
\begin{figure}
    ...
    \caption[
        \captionshort{Gamma prior for $\tau$ (blue) used for \software{bayesmeta} analyses of the day 1 versus baseline effect, compared to the empirical distribution of per-gene frequentist \software{metafor::rma.uni} estimates for $\tau$.}
    ]{
        \textbf{Gamma prior for $\tau$ (blue) used for \software{bayesmeta} analyses of the day 1 versus baseline effect, compared to the empirical distribution of per-gene frequentist \software{metafor::rma.uni} estimates for $\tau$.}
...
\begin{figure}
    ...
    \caption[
        \captionshort{Normal prior for $\mu$ (blue) used for \software{bayesmeta} analyses of the day 1 versus baseline \gls{DGE} effect, compared to the empirical distribution of per-gene frequentist \software{metafor::rma.uni} estimates for $\tau$.}
    ]{
        \textbf{Normal prior for $\mu$ (blue) used for \software{bayesmeta} analyses of the day 1 versus baseline \gls{DGE} effect, compared to the empirical distribution of per-gene frequentist \software{metafor::rma.uni} estimates for $\tau$.}
        Genes with small estimates of $\tau < 0.01$ were excluded before distribution fitting.
        The original non-scaled normal fit is shown (black), as well as a Cauchy fit (red).
        Distribution parameters are listed.
        An alternative estimate of the Normal standard deviation more robust to outliers using a quantile matching method from \software{DESeq2} \autocite{love2014ModeratedEstimationFold} is also given.
...
\subsection{Ranked gene set enrichment using blood transcription modules}
...
\begin{figure}
    ...
    \caption[
        \captionshort{Normalised gene expression for 857 genes differentially expressed between any pair of timepoints ($\text{lfsr} < 0.05$, $\lvert\text{\gls{FC}}\rvert > 1.5$).}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Transcriptomic modules up or downregulated between pairs of timepoints.}
    ]{
...
\subsubsection{Between-platform heterogeneity hinders detection of gene-level associations}
...
\begin{figure}
    ...
    \caption[
        \captionshort{\gls{DGE} effect sizes ($\log_2\text{\gls{FC}}$) estimated in array versus \gls{RNAseq} samples, colored by significance in frequentist random effects meta-analysis using \software{rma.uni} at $\text{\gls{BH} \gls{FDR}} < 0.05$.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{\gls{DGE} effect sizes ($\log_2\text{\gls{FC}}$) estimated in array versus \gls{RNAseq} samples, colored by significance in Bayesian random effects meta-analysis using \software{bayesmeta} at \software{ashr} $\text{\gls{lfsr}} < 0.05$.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Estimates of between-platform heterogeneity $\tau$ from frequentist and Bayesian meta-analysis, for the 58 genes with a significant association between day 7 expression and binary responder/non-responder status in \textcite{sobolev2016AdjuvantedInfluenzaH1N1Vaccination}.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Gene expression modules associated with antibody response (\gls{TRI}).}
    ]{
