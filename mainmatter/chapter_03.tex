...
\todo{actually, i've found that my PVE approximation is basically rescaled abs(Z), which is nonlinear rescaled lfsr, so pve is a bit pointless if we already have z}
...
As the sample size is greatest at day 0, 
...
\todo{requiring signif post-vaccination excludes many dampening effects}
% NOTE: the apparent gap is filled by dampening effects that are n.s. in day 1/7
Many of the \gls{reQTL} that satisfy this criteria have opposite effects pre- and post-vaccination (\autoref{fig:hird_eQTL_zSharing_vs_TSSdist_mega})---as lfsr quantifies uncertainty in the sign of the effect, I do not compare the sign unless the lfsr < 0.05 at day 0 also.
% Signs are only compared if both conditions have lfsr < 0.05, as the sign of the insignificant condition is uncertain.
...
% Gene set enrichment analysis on eGenes for these sets of \glspl{reQTL} at day 1 (88 eGenes) and day 7 (269 eGenes) did not detect any significant enrichments (\software{gprofiler2}, g:SCS adjusted p < 0.05).
