...
then applies shrinkage to provide improved posterior effect size estimates and compute measures of significance per condition. 
...
The boxed facets in \cref{fig:hird_reQTL_expressionTransform_sims} represent undesirable effects of transformations on \gls{reQTL} calls.
...
I estimated kinship in the \gls{HIRD} data from common autosomal variants, using {LDAK} (5.0) \autocite{speed2012ImprovedHeritabilityEstimation}, which computes \glspl{SNP}-based kinship matrices, weighting \glspl{SNP} by \gls{LD} and accounting for genotype uncertainty.
...
Note that although PEER factors are correlated with xCell scores (\cref{fig:hird_peer_corMatrix_v2_mega}), 
...
Here, the \gls{reQTL} variant explained similar amounts of expression variation at day 0 (\gls{PVE}=\percentage{0.08229266}) and day 7 (\gls{PVE}=\percentage{0.08956865}), with opposite directions of effect (\cref{fig:hird_reQTL_ploteQTL_SH2D4A}).
