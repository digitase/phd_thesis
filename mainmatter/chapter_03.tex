...
They attempted a model comparison with the \gls{CIT} \autocite{millstein2009DisentanglingMolecularRelationships} to resolve the directionality of association between expression and \gls{TRI}, but unfortunately concluded there was only \textapprox{\SI{60}{\percent}} power at the total sample size of $n=247$.
...
A myriad of approaches to mapping \glspl{reQTL} with linear models exist; each with its own advantages, disadvantages, and assumptions.
...
compute se
%
\software{HyPrColoc} implements a branch and bound divisive clustering algorithm. 
...
% If interest lies in identifying sets of colocalized traits above a certain posterior probability, more stringent regional and alignment threshold parameters can be chosen.
%
% - the posterior probability that these traits are colocalized
% - a 'large' regional association probability is evidence that one or more snps in the region have shared associations across the traits, always > posterior
% - how much of the posterior the candidate SNP explains
...
Gene set over-representation analysis on the set of 221 eGenes was relatively uninformative, and revealed only one enrichment for genes \gene{PRKACB}, \gene{PRKACA}, \gene{SAR1B} and \gene{APOE} in \enquote{Plasma lipoprotein assembly} (Reactome pathway identifer R-HSA-8963898, set size = 11, adj. p = \num{0.00691834}).
...
The presence of \gls{reQTL} without \gls{DGE} is exemplified at the strongest and only significant \gls{reQTL} at day 1 at FDR < 0.2 (\cref{fig:hird_eQTL_ploteQTL_ADCY3}).
...
% Overall, only 5/68 (\percentage{0.1323529}) genes with \glspl{reQTL} that explain more variation at day 1 were upregulated at day 1 vs. day 0.
% ; 5/226 (\percentage{0.02212389}) for day 7 vs. day 0.
...
\subsection{Colocalisation with external QTL datasets at the \gene{ADCY3} locus}
...
\begin{figure}
    ...
    \caption{
        \textbf{Expression of \gene{ADCY3} in immune cell types from the DICE database (\textcite{schmiedel2018ImpactGeneticPolymorphisms}).}
    }
...
\begin{figure}
    ...
    \caption{
        ...
        Default priors used, except \software{prior.2} = 0.99.
    }
...
No colocalisation with monocyte count was observed, so association between genotype and \gene{ADCY3} expression mediated by monocyte abundance is not implicated, 
...
% bait:
% map reqtl
% test some mechanisms
% at adcy3 pin down to naive monocytes
...
% TODO: why important? small and opposite effects may be more prevalant than expected
% best to detect them in a joint framework like mashr
...
% TODO: segue here
% A less paltable is these are false positives
...
% TODO: this is a first step towards causality of delta expression
...
% TODO: need ASE, 2 things are sus: no positional enrichment, they are weak
...
% TODO: add tmod section to method
% add gprinfiler

\todo{re: the original issue of discussing only top hits: hard to resolve due to the lack of gene set enrichments and time, but hopefully talking about sets of genes with shared generating mechanisms might work}
