...
Candidate gene studies and \gls{GWAS} have identified multiple genetic associations with antibody response \autocite{mooney2013SystemsImmunogeneticsVaccines,oconnor2013CharacterizingVaccineResponses,mentzer2015SearchingHumanGenetic,linnik2016ImpactHostGenetic}, 
...
\textcite{poland2008ImmunogeneticsSeasonalInfluenza} ($n=184$) tested HLA alleles, and \glspl{SNP} in coding and regulatory regions of cytokine or cytokine receptor genes for association with post-\gls{TIV} \gls{HAI} titres specific to H1 and H3 subtypes.
...
The associations listed above are in small cohorts and have not been validated by subsequent studies,
so it remains unknown whether robust genetic associations with antibody response to influenza vaccines exist.
\todo{no gwas studies that i can find}
...
As reviewed in \cref{subsec:intro_contextDependenteQTL} and \cref{subsec:intro_reQTL}, the effect sizes of \glspl{eQTL} can be highly context-dependent,
...
and even less work has been done where the stimulation is vaccination,
despite the potential for learning about the molecular mechanisms underlying variation in antibody responses.
...
\textcite{franco2013IntegrativeGenomicAnalysis} enrolled healthy Europeans adults into discovery ($n=119$ males) and validation ($n=128$ females) cohorts in two consecutive influenza seasons.
...
Genotyping was performed on genotyping arrays.

They mapped \textit{cis}-\gls{eQTL} with a linear mixed model over all four days,
including day, genotype, day-genotype interaction, and a random intercept for individual as predictors; and gene expression as response.
...
To call \glspl{reQTL} they also mapped \glspl{eQTL} separately at each day with a linear model including only genotype as a predictor and expression as response,
from which they computed the model $R^2$ as a rough measure of the variance in expression explained by the \gls{SNP} at each day.
They then computed delta-$R^2$: the maximum absolute deviation of any of the three post-vaccination $R^2$s from the day 0 $R^2$.
...
The union of the 78 and 34 genes from the above analyses (98 genes) was enriched for pathways and gene sets related to  
...
they filtered down to 20 genes with an \gls{eQTL}, 
expression correlation with \gls{TRI} at any day, 
and either post-vaccination differential expression \emph{or} a \gls{reQTL} effect.
...
Critically, \textcite{franco2013IntegrativeGenomicAnalysis} realised that just applying multiple filtering criteria does not allow them to infer the direction of causal relationships between genetic variation, expression and \gls{TRI}.
They attempted a model comparison with the \gls{CIT} \autocite{millstein2009DisentanglingMolecularRelationships} to resolve the directionality of association between expression and \gls{TRI}, but unfortunately concluded they only had \textapprox{\SI{60}{\percent}} power at their total sample size of $n=247$.
Nevertheless, the study is proof of concept that integration of genotype, expression and antibody response data in an \textit{in vivo} \gls{reQTL} framework can highlight genes under genetic regulation likely to be involved in vaccine response.
...
% TODO:
% WHY: cause you can't separate correlated types
...
% TODO: expected? in fairfax2012GeneticsGeneExpression , how many shared? in vivo studies, more or less?
...
\cref{fig:hird_eQTL_upset_mega} illustrates the difference between calling sharing using a significance threshold versus difference in betas approach.
...
and 1002 reQTL between day 7 and day 0 (\cref{fig:hird_eQTL_zSharing_vs_TSSdist_mega}).
...
where the \gls{reQTL} variant explained approximately \percentage{0.01862996} of expression variation at day 0, increasing to \percentage{0.14075782} at day 1 (\cref{fig:hird_eQTL_ploteQTL_ADCY3}).
At day 7 the strongest \gls{reQTL} was at \gene{SH2D4A} (p difference = \num{1.369564e-06}, BH FDR = \num{0.01747935}).
Here, the \gls{reQTL} variant explained similar amounts of expression variation at day 0 (\percentage{0.08229266}) and day 7 (\percentage{0.08956865}), with opposite directions of effect (\cref{fig:hird_eQTL_ploteQTL_SH2D4A}).
...
As described in \cref{subsec:hird_reQTL_xCell}, \software{xCell} enrichment scores were used to approximate abundance of seven \gls{PBMC} cell types from the expression data.
...
The small magnitude of the genotype main effect in the full model vs. the nested model indicates the \gls{eQTL} effect is driven largely by the monocyte score (or a cell type that is highly correlated with monocyte score, see \cref{fig:hird_xCell_cos2}).
...
\1 Day 1 HIRD colocs with BLUEPRINT and Fairfax monocytes (both stim and non stim), but not with Quach or Schmiedel monocytes (\cref{fig:hird_eQTL_coloc_ADCY3})
...
% TODO: but do we see the upreg they see here?
...
% TODO:
% but see this: enrichment in DE genes vs eQTLs and non eQTLs (Barreiro et al., 2012)
% and also see what franco said
%
% TODO
