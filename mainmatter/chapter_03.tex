...
\begin{figure}
    ...
    \caption{
        \textbf{
            Contribution of each cell type score to each \gls{PC} dimension after \gls{PCA} of standardised \software{xCell} cell type enrichment scores. 
        }
...
\software{tmod::tmodHGtest}, which implements the hypergeometric test for enrichment in \glspl{BTM}, controlling the \gls{FDR} at 0.05 using the \gls{BH} procedure.
...
For example, there were 85 \gls{eQTL}-eGene pairs significant only at day 1 post-vaccination ($\gls{lfsr} < 0.05$); of these only \num{40/85} are \glspl{reQTL} by the difference in betas method.
The difference in betas method is more strict because calling by significance alone would call a \gls{reQTL} for an \gls{eQTL} with \gls{lfsr} = 0.049 at baseline and \gls{lfsr} = 0.051 at day 1, even if the effect sizes are similar.
...
\begin{figure}
    ...
    \caption{
        ...
        Significant eQTLs (\gls{lfsr} < 0.05) were found at \num{6887/13570} eGenes.
...
The classifications are shown in \cref{fig:hird_reQTL_zSharing_vs_TSSdist_mega}, plotting all 6887 shared or \gls{reQTL} by their distance relative to the eGene \gls{TSS}.
...
A nominal \pvalue{} < 0.05 threshold, although stricter than many types of threshold (see \cref{subsec:hird_reQTL_methods_definingSharedAndreQTLs}), may still be too lax for calling \glspl{reQTL},
...
\cref{fig:hird_reQTL_zSharing_vs_TSSdist_mega} also shows whether each eGene was up or downregulated at the timepoint based on the \gls{DGE} analyses in \cref{ch:hird_DGE}.
...
\enquote{mitotic cell cycle} (\gls{GO} biological process term GO:0000278, term size = 914, intersection size = 12, \software{gprofiler2::gost} adj. p = \num{0.0001419083}),
...
The only significant \gls{reQTL} on day 1 at FDR < 0.2 was for \gene{ADCY3} (\cref{fig:hird_reQTL_ploteQTL_ADCY3}).
At day 1, the \gls{PVE} for this \gls{reQTL} associated with \gene{ADCY3} explained \percentage{0.01862996} of expression variation at baseline, increasing to \percentage{0.14075782} at day 1,
yet the gene was not differentially expressed from baseline to day 1 (log2FC=\num{0.10373360}, \gls{lfsr}=\num{0.2569459}).
...
Here, the \gls{reQTL} variant explained similar amounts of expression variation at baseline (\gls{PVE}=\percentage{0.08229266}) and day 7 (\gls{PVE}=\percentage{0.08956865}), with opposite directions of effect (\cref{fig:hird_reQTL_ploteQTL_SH2D4A}).
...
The high contributions to the same \gls{PC} for monocyte, CD4\textsuperscript{+} T cell, and CD8\textsuperscript{+} T cell \software{xCell} scores in the \gls{PCA} \software{xCell} scores indicates cell types with high \gene{ADCY3} expression are indeed correlated in \gls{HIRD} (\cref{fig:hird_xCell_cos2}).
...
% 45756 downstream
%
The value of \texttt{prior.2} (the probability that a variant associated with at least one trait is not associated with any additional traits) was subsequently set to 0.99 (default = 0.98) to increase stringency, preventing clustering of baseline \gls{HIRD} expression with T cell and \gls{IBD}.
...
\begin{figure}
    ...
    \caption{
        ...
        Colocalisation performed using {HyPrColoc} \autocite{foley2019FastEfficientColocalization} in a 500 Kb window around the lead variant for the day 1 \gene{ADCY3} \gls{reQTL} in \gls{HIRD} for trait datasets described in \cref{subsec:hird_reQTL_coloc}.
...
\begin{figure}
    ...
    \caption{
        ...
        Colocalisation performed using {HyPrColoc} \autocite{foley2019FastEfficientColocalization} in a 500 Kb window around the lead variant for the day 1 \gene{ADCY3} \gls{reQTL} in \gls{HIRD} (vertical grey line).
...
and 211/1002 (\percentage{0.2105788}) for day 7 \gls{reQTL} (\cref{fig:hird_reQTL_zSharing_vs_TSSdist_mega}).
...
Those day 7 \gls{reQTL} that were also upregulated at day 7 were enriched in cell cycle \gls{GO} sets, but it is unclear how this may lead to generation of opposite effects, and the enrichment may largely be driven by the \gls{DGE} signal rather than the \gls{reQTL} one.
...
While it is known that stimulation-specific \gls{reQTL} are more distal than baseline eQTL \autocite{fairfax2014InnateImmuneActivity}, the \gls{HIRD} reQTL are evenly spread across the \textit{cis}- window.
...
Compared to FACS measurements in a cohort subset, the \software{xCell} scores were only weakly correlated, and the signatures used by \software{xCell} may be less accurate after vaccine stimulation.
...
The proportion of monocytes in the \gls{PBMC} compartment increases at day 1, supported by both FACS \autocite{sobolev2016AdjuvantedInfluenzaH1N1Vaccination} measurements, and an increase in monocyte \software{xCell} score.
...
not helped by the unclear reliability of \software{xCell} scores,
...
One type of mechanism by which \textit{cis}-eQTL affect expression is through their impact on \gls{TF} binding affinity to motifs in promoters and enhancers \autocite{pai2015GeneticMechanisticBasis}.
