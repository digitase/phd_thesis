...
It is feasible that the mechanism generating \gls{reQTL} at the remainder of these genes involves cell type-specific \gls{eQTL} effects, although I have not yet determined the likely cell types at genes other than \gene{ADCY3}.
...
Aside from monocytes, \gene{ADCY3} is also expressed a wide range of immune cells: CD4\textsuperscript{+} T cells, CD8\textsuperscript{+} T cells, B cells, and \gls{NK} cells (\cref{fig:hird_eQTL_ADCY3_expression_DICE}).

The \gene{ADCY3} locus 
has not only
but
implicated in disease risk for \glspl{IMID}

The genotype by cell type interaction model cannot distinguish between cell types with highly correlated abundances \autocite{kim-hellmuth2020CellTypeSpecific},
and the high contributions to the same \gls{PC} monocyte, CD4\textsuperscript{+} T cell, and CD8\textsuperscript{+} T cell xCell scores indicate these abundances are correlated in \gls{HIRD} (\cref{fig:hird_xCell_cos2}).

To try and resolve cell type
and see shared cell type contexts with IMID
coloc
