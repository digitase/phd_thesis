...
Prior to imputation, 213277 monomorphic variants that provide no information for imputation were removed.
...
Poorly-imputed variants with $\text{INFO} < 0.4$ or post-imputation missingness $> 5\%$ were removed, resulting in 40290981 variants.
...
where array batch and \gls{RNAseq} plate were given as known batch effects.
...
In this chapter, I apply \software{mashr}\autocite{urbut2018FlexibleStatisticalMethods} for the estimation of \gls{eQTL} effects across my three timepoint conditions.
...
As discussed in \autoref{ch:hird_DGE}, there is population structure due to ancestry in the \gls{HIRD} cohort, which can affect gene expression\autocite{brown2018ExpressionReflectsPopulation}.
In \gls{DGE} analyses, I addressed this by treating the top \glspl{PC} of the genotype matrix as covariates for large-scale population structure.
In the context of \gls{eQTL} mapping, where the aim is to assess the marginal effect of a single genetic variant on expression, it is even more important that the confounding effect of population structure is properly controlled, or test statistics may be inflated.
...
An efficient approach is the \gls{LMM}, which includes a random effect that incorporates genetic correlation between individuals, usually in the form of a kinship matrix, into the covariance of that random effect\autocite{price2010NewApproachesPopulation, eu-ahsunthornwattana2014ComparisonMethodsAccount, golan2018MixedModelsCaseControl}
...
%
When testing a variant for association using \glspl{LMM}, to avoid loss of power from 'proximal contamination', the kinship matrix used should not include that variant\autocite{listgarten2012ImprovedLinearMixed}.
...
such as the rank-based \gls{INT} (e.g. used by GTEx v8\autocite{aguet2019GTExConsortiumAtlas}),
...
Another common transform is standardising (centering and scaling to zero mean and unit variance), recommended by the eQTLGen Consortium\autocite{vosa2018UnravelingPolygenicArchitecture}.
This is often done so that effects across all genes and studies can be comparably interpreted in units of standard deviation expression\autocite{qi2018IdentifyingGeneTargets}.
...
Note that many preprocessing steps done prior to this stage in the pipeline (e.g. variance-stabilisation, ComBat batch effect correction) are expression transformations.
I only consider the preservation of \gls{reQTL} effects defined from expression values post adjustment for those technical effects to be important.
...
which shift after Pandemrix vaccination\autocite{sobolev2016AdjuvantedInfluenzaH1N1Vaccination}.
\gls{eQTL} effects can also be cell type specific\todo{determine appropriate citations from existing refs in intro}.
...
Cell type abundance directly measured via \gls{FACS} are only available for a small subset of \gls{HIRD} individuals, so I derived cell type abundance estimates from the expression data as an alternative.
Such estimates have been used as predictors in \gls{eQTL} analysis in bulk samples where cell type specific effects are expected\autocite{westra2015CellSpecificEQTL,davenport2018DiscoveringVivoCytokineeQTL,kim-hellmuth2019CellTypeSpecific}.
...
then selected only one cell type with the highest contribution to each of those components.
...
As recommended by \autocite{stegle2012UsingProbabilisticEstimation}, between-sample normalisation and variance stabilisation \gls{RNAseq} data was performed using \software{DESeq2::vsn},
...
Note that I did not leave this adjustment for PEER to perform, as ComBat estimates centering and scaling factors per gene to adjust for batch effects, whereas the use of PEER factors represent a mean-only adjustment, which given the severity of the batch effect in this dataset (e \autoref{fig:hird_expression_pcs}), may be insufficient for strong batch effects\autocite{zhang2018AlternativeEmpiricalBayes}.
...
I map \glspl{eQTL} within each timepoint using \software{LIMIX}\autocite{lippert2014LIMIXGeneticAnalysis}, which implements efficient univariate and multivariate \glspl{LMM} with one or more random effects.
...
\todo{xchrom}
At each of 13570 genes, at all cis-variants within within +-1 Mb of the \gls{TSS}, I fit the following model to map \gls{eQTL}:
\begin{equation}
\begin{split}
...
where the \gls{eQTL} effect size of interest is the slope of the genotype fixed effect $\beta$,
and $\mathbf{u}$ is a random effect with zero mean and covariance matrix proportional to the \gls{LOCO} kinship matrix\footnote{For chromosome X variants, the \gls{LOCO} matrix for chromosome 1 is used.}.
% The LMM requires a kinship matrix to scale the covariance matrix of the random effect mean zero and an unknown variance.
\todo{add formulation of the 0-mean random effect to show exactly how the kinship matrix is used}
...
% Also see \autocite{demissie2011BiasDueTwostage}
...
Joint analysis was conducted with \software{mashr}\autocite{urbut2018FlexibleStatisticalMethods}, at 40197618 gene-variant pairs (tests) for which summary statistics from within timepoint mapping were available in all three timepoint conditions.
\todo{did I do a SNPs only filter? might have deprecated that}
...
I took the most significant variant per gene per condition for my strong subset (45962 tests), which ensures strong condition-specific effects are included (\autoref{fig:hird_mashr_strongSubset_Z_mega}, then further filtered to only nominally significant tests.
...
Thresholding on the lfsr is not appropriate for determining sharing, as the difference between significant and non-significant effect estimates in two conditions is not necessarily significant\autocite{gelman2006DifferenceSignificantNot}.
...
but this does not consider the posterior standard error around the estimates.
% Also see:
%
...
% Notes from 2018-10-11 on wald test, and comments on sharing_func in get sharing script
% Also: https://onlinelibrary.wiley.com/doi/abs/10.1111/j.1745-9125.1998.tb01268.x
...
It assumes posterior pairwise covariance of effects is zero $\sigma^2(x, y)$.
...
The z score can be compared to a standard normal to obtain a nominal Wald test p value, the null hypothesis being a difference of zero.
I use nominal p value < 0.05 as a heuristic threshold to define reQTL effects that are strong (like the mashr recommended 2-fold threshold), rather than a formal measure of significant difference.
...
I applied \software{qvalue::qvalue\_truncp} to estimate the proportion of those GTEx nominal p values that are null ($\pi_0$), the compute a measure of replication $\pi_1 = 1 - \pi_0$.
...
Past this, as the $\pi_1$ procedure assumes a well-behaved p value distribution in $\left[0, 1\right]$, 
reliability declines due to the number of p values being too small\footnote{\url{https://github.com/StoreyLab/qvalue/pull/6\#commitcomment-26277751}}, or the maximum p value being too far from 1.
...
As the mega-analysis has a higher eGene rate \percentage{0.5075166} vs. \todo{about 30pc, get exact figure} compared to the \gls{RNAseq}-only analysis, with similar replication,
...
\todo{Need to consider Niko's comment that there are too many genotype-platform interactions to use mega-analysis. Consider filtering.}

\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth,page=1]{mainmatter/figures/chapter_03/compute_pi1.pi1_by_thresholds.pdf}
    \caption{
        Effect of \gls{HIRD} lfsr threshold on GTEx whole blood replication rate ($\pi_1$), number of p values used to compute $\pi_1$, and maximum p value among those p values; 
...
\todo{add note here that although peer is correlated with xcell, interactions are only formed with xcell, so the interaction term can be interpreted per unit of genotype increase when xcell=0}
...
The strongest \glspl{eQTL} with the highest \gls{PVE} are shared between timepoints, highlighting the power advantage for mapping shared effects granted by joint analysis.
...
\todo{actually, i've found that my PVE approximation is basically rescaled abs(Z), so pve is a bit pointless if we already have z, and doesn't really help with comparability}
...
and are significant at the corresponding timepoint (\autoref{fig:hird_eQTL_zSharing_vs_TSSdist_mega}).
\todo{requiring signif post-vaccination may not be correct, as it excludes many dampening effects}
819 reQTL between day 1 and day 0, and 1002 reQTL between day 7 and day 0.
...
This indicates the \gls{eQTL} effect is actually driven largely by the monocyte score (or a cell type that is highly correlated with monocyte score, see \autoref{fig:hird_xCell_cos2}).
...
\1 \software{hyprcoloc} fine maps the signal to rs13407914 (PP = 1), an intronic variant 45064 bp downstream of the TSS.
...
\begin{figure}
    ...
    \caption{Multi-trait colocalisation of HIRD reQTL signal at ADCY3 (500 Kb window), with genetic associations from IHEC, BLUEPRINT, eQTL Catalogue, and GWAS Catalogue. Plots are colored by colocalised cluster. Black indicates non-colocalised datasets.}
...
Clearly, the enrichment of opposite sign effects in \gls{HIRD} is also most apparent at day 7.
\todo{I'm not exactly sure why at the moment.}
The strongest reQTL at day 7 is one such effect;
...
focusing only whether reQTLs that have a detectable main effect may be driven by cell type interactions.
%
...
Nevertheless, the xCell scores were used as a best approximation, as assuming a single genotype where cell-type specific slopes are likely is inappropriate.
%
...
and immune cells, including monocytes, are regulated by cell type specific \glspl{TF}\autocite{choudhury2016IdentifyingCellTypeSpecific}.
...
Rather than condition-specific expression of the eGene, what may be condition-specific is the expression of \glspl{TF} whose activity is affected by the reQTL \footnote{
...
\todo{Is this really a genotype-dependent change in gene expression between timepoints? i.e. how does the change score/ANCOVA approach differ from repeated measures ANOVA differ from the interaction/stratified approach I take?}
but have not attempted to determine resulting genotype-dependent differences in antibody phenotypes.
...
but a formal test such as the \gls{CIT}\autocite{millstein2009DisentanglingMolecularRelationships} is required to distinguish mediation of genotype-antibody associations through gene expression from competing models.
...
The \gls{HIRD} cohort is also too small for a direct \gls{GWAS} to Pandemrix antibody response.
