...
At each of \gene{13570} genes, at all cis-variants within within $\pm \SI{1}{\mega\bp}$ of the gene \gls{TSS}, I fit the following model to map \gls{eQTL}:
...
To optimise the number of factors to include as covariates%
...
\begin{figure}
    ...
    \caption{
        ...
        \gls{FDR} computed with hierarchical Bonferroni-\gls{BH} \autocite{huang2018PowerFalseDiscovery} and threshold set at 0.05.
...
This was approximated using the following formula from \textcite{shim2015MultivariateGenomeWideAssociation}:
\begin{equation}
    \frac{\beta_p^2 2 p q}{\beta_p^2 2 p q + \sigma_p^2 2N p q}
\end{equation}
where $\beta_p$ is the posterior beta from mashr,
$\sigma_p$ is its posterior standard error,
