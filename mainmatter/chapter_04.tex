...
\subsection{Anti-TNF treatment failure}
...
\begin{figure}
    ...
    \caption[
        \captionshort{Sample size and study day distribution for \gls{PANTS} study \gls{RNAseq} samples, stratified by timepoint and study group.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Distribution of \gls{RNAseq} samples from each patient among timepoints.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Correlation matrix of variables measured in \gls{PANTS} that were considered as potential predictor variables.}
    ]{
...
%     \caption[
%     ]{
%         Top 12 expression PCs of filtered expression data
%     }
...
\begin{figure}
    ...
    \caption[
        \captionshort{Variance components analysis showing the distribution of per-gene percentage of variance in expression explained by each variable.}
    ]{
...
%     \caption[
%     ]{
%         Changes in cell proportions of 6 immune cell types over time
%     }
...
\begin{figure}
    ...
    \caption[
        \captionshort{1000 Genomes Project (1000G) samples and PANTS samples projected onto 1000G genotype PC1 and PC2 axes, colored by (a) superpopulation and (b) population.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Number of eGenes on chromosome 1 vs. number of PEER factors included in \gls{eQTL} mapping as covariates.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Volcano plots of \gls{DGE} between primary responders (PR) and non-responders at week 0; unadjusted (top row) and adjusted (bottom row) for cell composition; for infliximab (IFX), adalimumab (ADA), or with both drugs pooled.}
    ]{
...
%     \caption[
%     ]{
%         Unadjusted, normalised SIGLEC10 expression over time
%     }
...
\begin{figure}
    ...
    \caption[
        \captionshort{Top modules differentially expressed between primary responders (PR) and non-responders at week 0, unadjusted for cell composition.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Top modules differentially expressed between primary responders (PR) and non-responders at week 0, adjusted for cell composition.}
    ]{
...
%     \caption[
%     ]{
%         Unadjusted, normalised TREM1 expression over time
%     }
...
\begin{figure}
    ...
    \caption[
        \captionshort{Volcano plots of \gls{DGE} between primary responders (PR) and non-responders at week 14; unadjusted (top row) and adjusted (bottom row) for cell composition; for infliximab (IFX), adalimumab (ADA), or with both drugs pooled.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Top modules differentially expressed between primary responders (PR) and non-responders at week 14, unadjusted for cell composition.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Top modules differentially expressed between primary responders (PR) and non-responders at week 14, adjusted for cell composition.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Top modules differentially expressed between week 14 and week 0, unadjusted for cell composition.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Top modules differentially expressed between week 14 and week 0, adjusted for cell composition.}
    ]{
...
\begin{figure}
    ...
    % TODO: add module names
    \caption[
        \captionshort{\software{tmod} evidence plots showing interferon-related modules specifically upregulated from week 0 to week 14 in primary non-responders (PNR), but not in primary responders (PR).}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Gap statistic versus cluster number $k$.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Normalised expression over the timepoints for genes in the six identified clusters.}
    ]{
...
\begin{figure}
    ...
    \caption[
        \captionshort{Post-treatment \gls{eQTL} effect sizes at week 30 and week 54 compared to baseline effect sizes at week 0.}
    ]{
...
%     \caption[
%     ]{
%         Clustering of eQTL betas over the 4 visits
%     }
...
%     \caption[
%     ]{
%         Gene set enrichment using gprofileR for cluster 1 genes
%     }
...
\gene{KCNN3} is annotated to a plasma cell surface signature module (LI.S3 \autocite{li2013MolecularSignaturesAntibody}), 
...
In monocytes, \gene{SIGLEC10} gene expression is more specific to the CD16\textsuperscript{+} monocytes \autocite{martinez2009TranscriptomeHumanMonocyte},
...
Low drug levels at week 14 were in turn associated with non-response.
...
\gene{TREM1} is expressed in myeloid lineage cells such as monocytes and macrophages;
\textcite{villani2017SinglecellRNAseqReveals} reported that \gene{TREM1} expression is most specific to classical monocytes and a newly identified subtype within the intermediate monocytes (\enquote{Mono3}).
...
%
% Although this chapter is purely descriptive,
...
A much larger proportion of the transcriptome was associated with response after the induction period at week 14.
Module associations
...
% NOTE: TNF also has anti-inflammatory action via M1 macrophages.
% Apoptosis, also known as programmed cell death, is a biological response that occurs following the resolution of inflammation (Elmore 2007).
% Macrophages possess membrane-bound TNF allowing them to induce neutrophil cell death (Allenbach et al. 2006).
% These apoptotic neutrophils are then cleared away by tissueresident macrophages via phagocytosis (Poon et al. 2014).
%
% Yet somehow "Anti-TNF-α Therapy Suppresses Proinflammatory Activities of Mucosal Neutrophils in Inflammatory Bowel Disease"
% https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6282128/
%
% As "TNFα can exert opposing, concentration-dependent effects on neutrophils to either accelerate their apoptosis or enhance their survival."
% https://ashpublications.org/blood/article/111/2/878/103793/The-dual-effects-of-TNF-on-neutrophil-apoptosis
%
...
% NOTE: would need to look at anti-drug Ab levels specifically?
...
An example is the most significant single-gene association from cluster 1 in spline model, \gene{KREMEN1}, 
...
The cluster contains genes such as \gene{STAT1}, \gene{IRF1}, and \gene{TAP1} that are induced by both type I and type II interferons \autocite{liu2012SystematicIdentificationType}.
% TODO: add TREM1 plot
...
Finally, I attempted to determine if there were changes in genetic architecture of expression over time,
...
\gene{NMI} and \gene{EPSTI1} are both interferon-induced genes with significant \glspl{reQTL} that had their strongest effect size on expression at week 54.
...
as their posterior effect sizes from \software{mashr} were very different from the input effect sizes from the per-timepoint models.
...
% Of the 246 patients with a week 14 sample, there are 20 patients with no drug level measured at week 14.
% However, there are other patients that have no week 14 sample at all.
% Which in total cuts out 196/814 samples.
...
The effect of differential drop out in responders and non-responders has not been explored.
