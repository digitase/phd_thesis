...
\autoref{fig:multipants_pheno_corrplot} shows their correlation matrix.
...
\todo{I intend to omit the expression PCA. Unless I get time to implement a systematic way to select which variables to color in each plot.}
...
These included sex, library preparation protocol version, and smoking status, but also response status, a variable of interest,
...
The sample size is large compared to number of \glspl{df} lost by including predictors that may not be relevant for some genes.
...
The regression model is agnostic to causal relationships between variables,
...
The major influence I consider is including cell proportions.
\todo{The observational study design does not allow estimation of causal effects, so this is only assumed.}
The assumed causal relationship is response status \textrightarrow cell proportion \textrightarrow expression
...
For \gls{DGE} in this chapter, I fit separate models including and excluding cell proportions as predictors, but otherwise identical.
These have complementary interpretations.
In models without cell proportions, the effect size of response status could represent up or downregulation, 
